...
Talisman(
    ...
    content_security_policy={
        ...
        'script-src': [
            ...
            "'unsafe-eval'",  # Необходимо для FullCalendar
            "'unsafe-inline'"  # Для inline-скриптов
        ],
        # Остальные директивы остаются без изменений
    },
    force_https=False  # Если нужно для разработки
)
...
def index():
    ...
    sort_by = request.args.get('sort_by', 'date_added')  # По умолчанию — по дате
...
def report():
    ...
    try:
        ...
        for risk in risks:
            ...
            elif closed_time and start <= closed_time <= end:
                filtered_risks.append(risk)
    except Exception:
        filtered_risks = []
...
def export_report(format):
    ...
    if format == 'excel':
        ...
        try:
            ...
            print("Шрифт DejaVuSans.ttf не найден. Используется стандартный шрифт.")
...
def update_status(index):
    ...
    risks[index]['updated_at'] = datetime.now().strftime("%Y-%m-%d %H:%M")
